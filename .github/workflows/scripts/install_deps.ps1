...
if(-not (Test-Path $c))
{
    ...
    cmake -G "Visual Studio 17 2022" -S fmt -B fmt/build --parallel -DCMAKE_CXX_STANDARD=20 -DFMT_INSTALL=ON -DFMT_TEST=OFF -DCMAKE_INSTALL_PREFIX=C:/deps/fmt
    cd ./fmt/build
    cmake --build . --config $env:BUILD_TYPE
    cmake --build . --target INSTALL --config $env:BUILD_TYPE
    cd ../..
    ...
    cmake -G "Visual Studio 17 2022" -S Catch2 -B Catch2/build --parallel -DCMAKE_CXX_STANDARD=20 -DCATCH_INSTALL_DOCS=OFF -DCMAKE_INSTALL_PREFIX=C:/deps/Catch2
    cd ./Catch2/build
    cmake --build . --config $env:BUILD_TYPE
    cmake --build . --target INSTALL --config $env:BUILD_TYPE
    cd ../..
